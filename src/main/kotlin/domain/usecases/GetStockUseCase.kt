                        ...
                        async {
                            when (stockSource) {
                                is StockSource.CompaniesMarketCap -> {
                                    getCompaniesMarketCapUseCase.execute(stockSource.pageCount)
                                }

                                StockSource.Sp500 -> {
                                    slickChartsRemoteDataSource.fetchSp500().mapNotNull(SlickChartsResponse::symbol)
                                }

                                StockSource.Nasdaq -> {
                                    slickChartsRemoteDataSource.fetchNasdaq100().mapNotNull(SlickChartsResponse::symbol)
                                }
                            }
                        }
                    }.awaitAll()
